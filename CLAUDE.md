...
**Status:** All modules implemented. 1118 core tests, 281 schema tests, 56 geo tests, 193 connected tests, 71 language-service tests, 48 doctest tests, 35 examples.
...
- **An op these paths cannot reproduce is REJECTED, never silently reinterpreted.** They emit exactly one `Put`/`Delete` per op, so `internal/TransactableOps.rejectUnsupportedOp` fails with a `ValidationError` for: **`upsert`** (an `UpdateItem` with `if_not_exists` on `createdAt`/immutable fields/version — a `Put` resets all three), `refs` (hydration needs a read), `generatedId` (needs `Crypto`), and vector indexes (needs `Embedder`). `update`/`patch` were already rejected. `EntityPutImpl` carries `_putKind: "put" | "create" | "upsert"` to tell the three apart — **every combinator that reconstructs an `EntityPutImpl` MUST carry `_putKind` through**, or an upsert silently becomes a plain `Put` again (guarded by the `.condition()` preservation test).
- **Known gap — multi-item lifecycle features are NOT gated.** `unique`, `versioned: { retain: true }` and `softDelete` entities written through the transact/batch paths get no sentinel / snapshot / tombstone, because the path emits one item. Left as-is deliberately: long-shipped, exercised by the connected suite, and fixing it means emitting extra transact items, which collides with EventStore's position-based cancellation mapping (caller-visible `additionalItems` indices must not shift). Needs its own issue.
