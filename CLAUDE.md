...
**Status:** All modules implemented. 966 core tests, 281 schema tests, 56 geo tests, 141 connected tests, 71 language-service tests, 48 doctest tests, 35 examples.
...
- **Write gate.** put/create/upsert always embed; update/patch embed **only** when a `source.fields` member is in the payload (mirrors the §7 GSI gate) and read the current item once when the payload is a partial source. `.withVector(name, vector)` on `BoundPut`/`BoundUpdate` skips the Embedder. `reembed({ concurrency })` rewrites stale vectors.
