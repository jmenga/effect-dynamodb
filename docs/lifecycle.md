...
```typescript
yield* EmployeeEntity.delete({ employeeId: "emp-alice" })
```
...
|-------------|---------------------|
| `entity.get(key)` | No |
| `entity.query.byTenant(...)` | No |
| Collection queries | No |
| `entity.deleted.get(key)` | Yes |
| `entity.deleted.list()` | Yes |

### Accessing Deleted Items

```typescript
// Get a specific deleted item
const deleted = yield* EmployeeEntity.deleted.get({ employeeId: "emp-alice" })
...
const allDeleted = yield* EmployeeEntity.deleted.list().pipe(
  Query.limit(20),
  Query.reverse,
  Query.execute,
)
...
```typescript
yield* EmployeeEntity.restore({ employeeId: "emp-alice" })
```
...
```typescript
yield* EmployeeEntity.restore({ employeeId: "emp-alice" }).pipe(
...
```typescript
yield* EmployeeEntity.purge({ employeeId: "emp-alice" })
```
...
const v2 = yield* EmployeeEntity.getVersion({ employeeId: "emp-alice" }, 2)

// Browse version history (most recent first)
const history = yield* EmployeeEntity.versions({ employeeId: "emp-alice" }).pipe(
  Query.limit(10),
  Query.reverse,
  Query.execute,
)
...
const program = Effect.gen(function* () {
  // Create (version 1)
  const alice = yield* EmployeeEntity.put({
  ...
  // yield* EmployeeEntity.get({ employeeId: "emp-alice" })  // ItemNotFound!
  ...
  // Restore (version 4)
  const restored = yield* EmployeeEntity.restore({ employeeId: "emp-alice" })
  // restored.version === 4
...
- [Advanced](./advanced.md) — DynamoDB Streams, Entity.Marshalled, testing patterns
