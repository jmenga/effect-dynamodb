...
Call `DynamoClient.make(table)` to get a typed client with executable operations. The typed client includes a `createTable()` method that derives the physical table definition (primary key, GSIs) from your registered entities — no manual `CreateTable` calls needed.
...
const program = Effect.gen(function* () {
  ...
  // Create
  const todo = yield* db.TodoEntity.put({
    todoId: "t-1",
  ...
  // Read
  const found = yield* db.TodoEntity.get({ todoId: "t-1" })

  // Update
  yield* db.TodoEntity.update({ todoId: "t-1" }, Entity.set({ done: true }))

  // Delete
  yield* db.TodoEntity.delete({ todoId: "t-1" })
})
