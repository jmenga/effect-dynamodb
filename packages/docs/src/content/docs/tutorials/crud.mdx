...
|---------|---------------|
| **Typed client** | `const db = yield* DynamoClient.make(MainTable)` resolves dependencies; all ops have `R = never` |
| **Variadic combinator updates** | `db.Users.update(key, Entity.set(...), Entity.expectedVersion(n))` — combinators as rest args |
...
| **GSI queries** | `Users.query.byRole(composites)` on entity definition, `db.Users.collect(query)` on bound entity |
...
| **Version history** | `versioned: { retain: true }` creates snapshots; `db.Users.getVersion(key, n)` and `Users.versions(key)` for retrieval |
| **Soft delete + restore** | `softDelete: true` archives items; `db.Tasks.restore(key)` recomposes all keys |
| **Purge** | `db.Users.purge(key)` permanently removes item + versions + soft-deleted copies |
