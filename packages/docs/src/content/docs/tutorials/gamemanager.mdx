...
```bash
pnpm add effect@4.0.0-rc.112 @effect/platform-node@4.0.0-rc.112 effect-dynamodb ulid
pnpm add -D typescript@^5.9.0 tsx vitest@^4.1.0 @effect/vitest@4.0.0-rc.112 @biomejs/biome@^2.4.4
```
...
export class ApiNotFound extends Schema.Error<ApiNotFound>("ApiNotFound")({
  error: Schema.Literal("NotFound"),
  message: Schema.String,
}, { httpApiStatus: 404 }) {}

export class ApiBadRequest extends Schema.Error<ApiBadRequest>("ApiBadRequest")({
  error: Schema.Literal("BadRequest"),
  message: Schema.String,
}, { httpApiStatus: 400 }) {}

export class ApiConflict extends Schema.Error<ApiConflict>("ApiConflict")({
  error: Schema.Literal("Conflict"),
  message: Schema.String,
}, { httpApiStatus: 409 }) {}

export class ApiUnprocessableEntity extends Schema.Error<ApiUnprocessableEntity>("ApiUnprocessableEntity")({
  error: Schema.Literal("UnprocessableEntity"),
  message: Schema.String,
}, { httpApiStatus: 422 }) {}
```

> **Concept: Schema.Error for API errors** — Each error uses `Schema.Error` — the idiomatic Effect v4 pattern for HTTP API errors. The second argument sets class-level annotations including `httpApiStatus`. This gives you proper Error instances with constructors (`new ApiNotFound({ error: "NotFound", message: "..." })`), clean JSON responses (no `_tag` in the wire format), and status-code-based discrimination — the HttpApi client dispatches by HTTP status code, not by `_tag`, so each error just needs a unique status.
...
```ts title="src/models/SquadSelection.ts" region="model-squad-selection" example="cricket.ts"
...
```ts title="src/entities/SquadSelections.ts" region="entity-squad-selection" example="cricket.ts"
import { DynamoModel, Entity } from "effect-dynamodb"
...
```ts title="src/models/Match.ts" region="model-aggregate" example="cricket.ts"
import { DynamoModel } from "effect-dynamodb"
import { Schema } from "effect"
...
```ts title="src/entities/MatchAggregate.ts" region="aggregates" example="cricket.ts"
...
- **`collection`** — Names the item collection used to fetch every item belonging to one match (the aggregate's "get" query). The query is a bare partition-key lookup that discriminates items in memory, so no secondary index is needed — `name` alone is enough and the read runs against the base table. You would add `index` + `sk` only if the aggregate were keyed on something other than the table's partition key.
...
Update `create-table.ts` to register `MatchAggregate` alongside the entities. The library merges the runtime-registered aggregates into the supplied table when computing the create-time schema. `MatchAggregate` assembles off the base table, so it contributes no index of its own and `db.tables.MainTable.create()` provisions just the existing entity GSIs:
...
Notice how the create payload maps to the aggregate's edge structure. The flattened ref ID fields preserve the referenced entity's branded identifier type (just like `Entity.Input` and `createSchema`) — `MatchAggregate.createSchema` / `inputSchema` expose `venueId: VenueId`, `teamId: TeamId`, etc., not plain `string`:
- `venue: Venue` → `venueId: VenueId`
- `team1.team: Team` → `team1.teamId: TeamId`
- `team1.coach: Coach` → `team1.coachId: CoachId`
- `team1.players[].player: Player` → `team1.players[].playerId: PlayerId`
...
The response contains full `Team`, `Player`, `Coach`, and `Venue` objects — not IDs. This is a single `get` call that fetches every item sharing the match's partition key straight from the base table and assembles them into the domain graph.
...
| **Schema.Error API errors** | Typed HTTP errors with constructors, status codes, clean JSON | ApiErrors.ts |
