...
import { DynamoModel } from "@effect-dynamodb/core"
import { Schema } from "effect"
```
...
- **`DynamoClient.make(MainTable)`** — The typed execution gateway. Resolves `DynamoClient | TableConfig` from context, binds all entities registered on the table, and returns a typed client where every operation has `R = never`. Access bound entities via `db.Teams`, `db.Players`, etc.
...
- **`byPlayer` index** — GSI for "all squads a player has been selected for". PK by `playerId`, SK by `season` + `series`.
...
export class SquadSelectionService extends ServiceMap.Service<SquadSelectionService>()(
  "@gamemanager/SquadSelectionService",
  {
    make: Effect.gen(function* () {
      const db = yield* DynamoClient.make(MainTable)
      const squads = db.SquadSelections
      return {
        create: Effect.fn(function* (input: CreateSquadSelectionInput) {
          return yield* squads.put(input)
        }),

        get: (key: { squadId: string; selectionNumber: number }) => squads.get(key),

        delete: (key: { squadId: string; selectionNumber: number }) => squads.delete(key),

        listByPlayer: (playerId: string, pagination?: PaginationOptions) =>
          squads.fetch(
...
export class MatchService extends ServiceMap.Service<MatchService>()("@gamemanager/MatchService", {
  make: Effect.gen(function* () {
    const db = yield* DynamoClient.make(MainTable)
    const boundUmpires = db.Umpires
    const matches = db.MatchAggregate
    return {
...
# Verify the update
curl -s http://localhost:3000/teams/$AUS_ID | jq '.name'
# → "Australia XI"
