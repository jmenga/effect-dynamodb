...
export const GenderSchema = Schema.Literals(Object.values(Gender))
...
export const BattingHandSchema = Schema.Literals(Object.values(BattingHand))
...
export const MatchTypeSchema = Schema.Literals(Object.values(MatchType))
export type MatchType = typeof MatchTypeSchema.Type

// Case-insensitive query variants: accept "male" or "Male"
const lowercaseValues = <T extends Record<string, string>>(obj: T) =>
  Object.values(obj).map((v) => v.toLowerCase()) as Lowercase<T[keyof T]>[]

export const GenderQuerySchema = Schema.Literals(lowercaseValues(Gender))
export const GenderQuery = Schema.Union([GenderSchema, GenderQuerySchema])
```

> **Pattern: Enum objects with Schema.Literals** — Define enums as `const` objects for autocomplete and referenceability (`Gender.Male`), then derive a `Schema.Literals` for runtime validation. The `type` alias extracts the union type from the schema. This gives you three things: a namespace (`Gender.Male`), a schema (`GenderSchema`), and a type (`Gender`).

> **Pattern: Case-insensitive query schemas** — `GenderQuerySchema` accepts lowercase variants (`"male"`, `"female"`), then `Schema.Union` combines both. API query parameters like `?gender=male` or `?gender=Male` both pass validation. The `lowercaseValues` helper derives lowercase literals from the enum object with proper typing via `Lowercase<T>`. This works because effect-dynamodb widens query input types to accept lowercase variants of string literals (via `CaseInsensitive<T>`) and applies the schema's casing (default: `"lowercase"`) to composite key values — so `"Male"` and `"male"` both produce the same key `$schema#v1#team#male#...`. The actual attribute value stored on the item retains its original casing.
...
import { Coaches } from "./Coaches.js"
import { Players } from "./Players.js"
import { CricketSchema } from "./Schema.js"
import { Teams } from "./Teams.js"
import { Umpires } from "./Umpires.js"
import { Venues } from "./Venues.js"
