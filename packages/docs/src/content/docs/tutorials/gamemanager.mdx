...
Update `create-table.ts` to register `SquadSelections` so `db.tables.MainTable.create()` adds the new GSIs (gsi3 and gsi4 for cascade):
...
export class MatchService extends ServiceMap.Service<MatchService>()("@gamemanager/MatchService", {
  make: Effect.gen(function* () {
    ...
    const boundUmpires = db.entities.Umpires
...
- **`DynamoClient.make({ entities, aggregates, tables })`** — Both entities and aggregates are bound via the same typed gateway. Access bound entities through `db.entities.Umpires` and bound aggregates through `db.aggregates.MatchAggregate` — all operations have `R = never`.
...
> **Aggregates on the table** — Aggregates are registered on `Table.make(...)` (so `db.tables.MainTable.create()` knows about their GSIs) and listed under `aggregates` in `DynamoClient.make({ ..., aggregates: { MatchAggregate } })` (so the typed client exposes `db.aggregates.MatchAggregate`). All bound operations have `R = never`.
...
```ts
// In App.ts — add imports:
import type { AggregateAssemblyError, ItemNotFound, RefNotFound } from "effect-dynamodb"
import { Match } from "../models/Match.js"
...
Update `create-table.ts` to register `MatchAggregate` so `db.tables.MainTable.create()` adds the aggregate's collection GSI (`gsi2` with `pk` + `gsi2sk`) and list GSI (`gsi1` with `gsi1pk` + `gsi1sk`) alongside the entity GSIs:

```ts
// In create-table.ts — add the aggregates record
import { MatchAggregate } from "./entities/MatchAggregate.js"
