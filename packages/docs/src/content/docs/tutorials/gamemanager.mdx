...
> **Key concept:** `Entity.make` returns a **pure entity definition** — no table reference, no executable operations. The entity declares its model, primary key, and configuration. GSI access patterns are declared separately as Collections. Table association happens when you register it on a `Table.make({ entities: { Teams } })`, which we'll create next.
...
- **`SquadsByPlayer` collection** — Defined separately via `Collections.make`, this GSI is keyed by `playerId`, allowing queries like "find all squad selections for a given player". In the v2 API, GSI access patterns are declared as Collections rather than inline on the entity.
