...
- BoundQuery combinators: `.filter()`, `.limit()`, `.pageSize()`, `.consistentRead()` on scans
...
`.limit()` bounds the **result**: at most `n` items come back, however many requests that takes. To size the requests themselves — DynamoDB's `Limit` — use `.pageSize()`:
...
The complete runnable example is at [`examples/scan.ts`](https://github.com/effect-ts/effect-dynamodb/tree/main/packages/effect-dynamodb/examples/scan.ts) in the repository.
...
| **`.pageSize()`** | Caps the rows examined per DynamoDB request (`Limit`) |
