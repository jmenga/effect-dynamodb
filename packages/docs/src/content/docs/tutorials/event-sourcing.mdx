...
import { DynamoSchema, EventStore, Table } from "effect-dynamodb"
...
// Commands
type MatchCommand =
  | { readonly _tag: "StartMatch"; readonly venue: string; readonly homeTeam: string; readonly awayTeam: string }
  | { readonly _tag: "CompleteInnings"; readonly innings: number; readonly runs: number; readonly wickets: number }
  | { readonly _tag: "EndMatch"; readonly result: string }

// Domain errors — tagged for precise catchTag handling
...
// State: in-progress, Innings: 1, Version: 2
...
for (const event of allEvents) {
  console.log(`v${event.version}: ${event.eventType} at ${event.timestamp}`)
}
