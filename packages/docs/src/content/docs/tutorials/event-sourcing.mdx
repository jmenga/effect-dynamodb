...
Real event-sourced systems usually need one non-event record kept in step with the events: an ingestion watermark, a stream registry row, a uniqueness guard. `append` accepts `additionalItems` — the same operation builders `Transaction.transactWrite` takes (`Entity.put`, `Entity.delete`, and `Transaction.check`) — and commits them in the **same** `TransactWriteItems` call as the events:
...
| **Atomic side writes** | `append(..., { additionalItems })` commits `Entity.put` / `Entity.delete` / `Transaction.check` ops with the events |
