...
const VcsTable = Table.make({ schema: VcsSchema, entities: { Users, Repositories, Issues, PullRequests } })
```
...
const {
  Users: boundUsers,
  Repositories: boundRepos,
  Issues: boundIssues,
  PullRequests: boundPRs,
} = yield* DynamoClient.make(VcsTable)

// Seed all data
for (const user of Object.values(users)) {
  yield* boundUsers.put(user)
}
for (const repo of Object.values(repos)) {
  yield* boundRepos.put(repo)
}
for (const issue of Object.values(issues)) {
  yield* boundIssues.put(issue)
}
for (const pr of Object.values(pullRequests)) {
  yield* boundPRs.put(pr)
}
```
...
```typescript
const octocat = yield* boundUsers.get({ username: "octocat" })
// octocat.fullName → "The Octocat"
// octocat.bio → "GitHub mascot"
// octocat.location → "San Francisco, CA"

const torvalds = yield* boundUsers.get({ username: "torvalds" })
...
```typescript
const helloWorld = yield* boundRepos.get({
  repoOwner: "octocat",
  repoName: "hello-world",
})
...
const linux = yield* boundRepos.get({
  repoOwner: "torvalds",
  repoName: "linux",
})
...
```typescript
// Octocat's repositories
const octocatRepos = yield* boundRepos.collect(
  Repositories.query.created({ repoOwner: "octocat" }),
)
// → [{ repoName: "hello-world", ... }]

// Torvalds' repositories
const torvaldsRepos = yield* boundRepos.collect(
  Repositories.query.created({ repoOwner: "torvalds" }),
)
// → [{ repoName: "linux", ... }]

// User side of the same collection
const octocatProfile = yield* boundUsers.collect(
  Users.query.owned({ username: "octocat" }),
)
// → [{ fullName: "The Octocat", ... }]
```
...
```typescript
// Create a new issue
const newIssue = yield* boundIssues.put({
  issueNumber: "3",
  repoName: "hello-world",
  repoOwner: "octocat",
  username: "torvalds",
...
// Close the issue — status touches GSI1 and GSI2 sort keys
// GSI1 (created): username + status — both required
// GSI2 (todos): status is the only non-primary composite
const closedIssue = yield* boundIssues.update(
  { repoOwner: "octocat", repoName: "hello-world", issueNumber: "3" },
  { status: "Closed", username: "torvalds" }, // all GSI1 composites required
)
...
```typescript
// Create a PR
const newPR = yield* boundPRs.put({
  pullRequestNumber: "2",
  repoName: "hello-world",
  repoOwner: "octocat",
  username: "octocat",
...
// Close the PR (merged)
const closedPR = yield* boundPRs.update(
  { repoOwner: "octocat", repoName: "hello-world", pullRequestNumber: "2" },
  { status: "Closed", username: "octocat" }, // all GSI1 composites required
)
...
```typescript
// Torvalds' issues
const torvaldsIssues = yield* boundIssues.collect(
  Issues.query.created({ username: "torvalds" }),
)
// → [
//   { issueNumber: "1", subject: "Bug: README has typo", status: "Open" },
//   { issueNumber: "3", subject: "Enhancement: Add CI pipeline", status: "Closed" },
// ]

// Torvalds' PRs
const torvaldsPRs = yield* boundPRs.collect(
  PullRequests.query.created({ username: "torvalds" }),
)
// → [{ pullRequestNumber: "1", subject: "Fix README typo", status: "Open" }]

// Octocat's issues
const octocatIssues = yield* boundIssues.collect(
  Issues.query.created({ username: "octocat" }),
)
// → 2 issues (hello-world #2 Closed, linux #1 Open)

// Octocat's PRs
const octocatPRs = yield* boundPRs.collect(
...
```typescript
// hello-world issues
const hwIssues = yield* boundIssues.collect(
...
const linuxIssuesAfter = yield* boundIssues.collect(
  Issues.query.todos({ repoOwner: "torvalds", repoName: "linux" }),
)
// → 2 issues now

const linuxPRsAfter = yield* boundPRs.collect(
