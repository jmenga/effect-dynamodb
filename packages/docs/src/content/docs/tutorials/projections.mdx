...
:::caution[Hand-composed keys ahead]
The example below builds raw partition/sort key strings (`"$proj-demo#v1#employee#e-1"`) so it can call `client.getItem` directly. This couples your code to the **internal key format**, which is implementation-detail of the schema/casing/version layer and is not part of the public API contract. Don't do this in production code unless you're sure you need the raw SDK escape hatch — the typed equivalent (`db.entities.Employees.get({ employeeId: "e-1" })`) computes the same key safely. A future release will expose `Entity.keyFor(...)` so projections can be combined with typed key composition without dropping to the SDK.
:::
...
The complete runnable example is at [`examples/projections.ts`](https://github.com/effect-ts/effect-dynamodb/tree/main/packages/effect-dynamodb/examples/projections.ts) in the repository.
