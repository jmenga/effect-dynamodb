...
const { Users: users, ApiRequests: requests } = yield* DynamoClient.make(MainTable)
...
// Created: Alice (email: alice@example.com, username: alice)
...
yield* users.update({ userId: "u-1" }, Entity.set({ email: "alice-new@example.com" }))
...
yield* users.update(
  { userId: "u-2" },
  Entity.set({ email: "alice-new@example.com" }),
).pipe(
  Effect.catchTag("UniqueConstraintViolation", (e) =>
    Effect.succeed(`Blocked: constraint="${e.constraint}"`),
  ),
...
import { DynamoClient, Table } from "effect-dynamodb"
