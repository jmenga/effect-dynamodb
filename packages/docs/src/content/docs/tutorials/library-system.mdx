...
| `Categories` | GSI1 | genre | Genre (isolated) |
...
Author defines only a primary key by last name + first name:
...
Book defines only a primary key. The `loans` collection on GSI1 uses optional fields, creating a **sparse index**:
...
Genre defines only a primary key. It participates in three collection patterns plus a standalone category collection:
...
Member defines only a primary key. GSI1 participates in the `Account` collection alongside Book loans:
...
GSI access patterns are now defined as entity-level indexes. Multi-entity collections (account, works, titles) are auto-discovered from matching collection names across entities:
...
- **`Account` (GSI1)** uses `memberId` as PK. Book's `loanEndDate` is `Schema.optional`, so books without a loan are invisible in this index (sparse index pattern)
- **`Works` (GSI2)** groups Author, Book, and Genre by author name -- three entity types in one partition
- **`Titles` (GSI3)** groups Books and Genres by title
- **`Categories` (GSI1)** is `isolated` -- standalone genre browsing, not clustered with Account
...
The `Categories` collection on GSI1 is an **isolated** collection for standalone genre browsing. It uses `genre` as PK and `subgenre` as SK for hierarchical browsing.
