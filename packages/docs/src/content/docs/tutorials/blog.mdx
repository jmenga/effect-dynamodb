...
`yield*` on a `put` or `get` returns the clean model type. `Entity.asRecord` — which exposes system fields like `createdAt`, `updatedAt` and `version` — is a combinator on the **unbound** entity descriptor, so pipe the entity's own `get` through it, not the bound client's:
...
// Entity.asRecord exposes system fields — on the unbound descriptor
...
The complete runnable example is at [`examples/blog.ts`](https://github.com/effect-ts/effect-dynamodb/tree/main/packages/effect-dynamodb/examples/blog.ts) in the repository. It creates all entities, demonstrates each access pattern, and cleans up after itself.
