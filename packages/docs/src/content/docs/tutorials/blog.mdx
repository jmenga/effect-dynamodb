...
- GSI queries for posts by author and comments by post via Collections
...
3. **Posts by author** -- Find all posts by a user (GSI1: `PostsByAuthor` collection)
4. **Comments on a post** -- Find all comments on a post (GSI1: `CommentsByPost` collection)
...
Posts have a primary key. The GSI for querying by author is defined in the `PostsByAuthor` collection:
...
### Comment Entity and Collections

Comments also use GSI1 but with a different partition key -- this is **index overloading**. Posts partition GSI1 by `authorId` (via `PostsByAuthor`), while Comments partition it by `postId` (via `CommentsByPost`):
...
| **Post** | PostsByAuthor (GSI1) | `authorId` | `postId` | Posts by author |
| **Comment** | primary | `commentId` | (empty) | Get comment by ID |
| **Comment** | CommentsByPost (GSI1) | `postId` | `commentId` | Comments on a post |

GSI1 is **overloaded** -- Posts use it for `PostsByAuthor` (partitioned by `authorId`) and Comments use it for `CommentsByPost` (partitioned by `postId`). Different entity types, different partition keys, same physical index.
...
Query posts by author and comments by post using Collections:
