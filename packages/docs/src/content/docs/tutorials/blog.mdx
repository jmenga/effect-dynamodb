...
- **`status`** uses `Schema.Literals` for a constrained set of post states
...
const { Users: users, Posts: posts, Comments: comments } = yield* DynamoClient.make(BlogTable)
...
```typescript
// All posts by alice
const alicePosts = yield* posts.collect(Posts.query.byAuthor({ authorId: "alice-1" }))
// → [
//   { postId: "post-1", title: "Getting Started with Effect TS", ... },
//   { postId: "post-2", title: "DynamoDB Single-Table Design", ... },
//   { postId: "post-3", title: "Draft Post", ... },
// ]

// All comments on post-1
const postComments = yield* comments.collect(
  Comments.query.byPost({ postId: "post-1" }),
)
// → [
//   { commentId: "comment-1", body: "Great article!", ... },
//   { commentId: "comment-2", body: "Very helpful, thanks!", ... },
// ]
```
...
const updatedAlice = yield* users.update(
  { userId: "alice-1" },
  { displayName: "Alice B.", postCount: 3 },
)
// updatedAlice.displayName → "Alice B."
// updatedAlice.postCount → 3
```
...
```typescript
// Alice is currently at version 3 after two updates
const lockResult = yield* users.update(
  { userId: "alice-1" },
  Entity.set({ displayName: "Wrong Name" }),
  Entity.expectedVersion(1), // stale version — will fail
).pipe(
  Effect.catchTag("OptimisticLockError", (e) =>
    Effect.succeed(`Caught: expected v${e.expectedVersion}`),
  ),
)
// → "Caught: expected v1"
```
...
// Atomic read — typed tuple
const [txUser, txPost] = yield* Transaction.transactGet([
  Users.get({ userId: "alice-1" }),
  Posts.get({ postId: "post-1" }),
])
// txUser: User | undefined
// txPost: Post | undefined

// Atomic write — create + delete in one transaction
