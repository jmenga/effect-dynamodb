...
Both entities share a single table. Orders have a GSI for querying by user:
...
// 3 users + 3 orders created in a single batch call
```
...
// alice: User | undefined — fully typed as User
// bob: User | undefined
// order1: Order | undefined — fully typed as Order
// order2: Order | undefined
```
...
// existing → { userId: "u-1", name: "Alice", ... }
// missing → undefined
```
...
// Added o-4 and deleted o-3 in one atomic batch
...
// deleted → undefined (removed)
// created → { orderId: "o-4", product: "Thingamajig", ... }
```
...
// Created user u-4 + order o-5, deleted user u-3 — all in one batch
```
