...
Both spellings unwrap through the same protocol, so an array can mix them freely. Anything that is *not* a get descriptor — an already-executed `Effect`, a query, a write op — fails with a `ValidationError` carrying **EDD-9052** rather than a defect you cannot catch.
:::
