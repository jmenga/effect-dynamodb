...
| Query by access pattern | `entity.query.<name>(pk).go()` | `bound.collect(EntityDef.query.<name>(pk))` | Parity |
...
| Scan | `entity.scan.go()` | `bound.collect(EntityDef.scan())` | Parity |
...
| Update | `entity.update(pk).set({}).go()` | `bound.update(key, Entity.set(updates))` | Parity |
| Patch (fail if not exists) | `entity.patch(pk).set({}).go()` | `bound.patch(key, Entity.set(updates))` | Parity |
...
| Conditional writes | `.where()` on any mutation | `Entity.condition(callback \| shorthand)` on put/update/delete | Parity |
...
| Data callback | `.data((attrs, ops) => ...)` | Not supported — use pipe composition with Effect operators | ElectroDB only |
| Nested property updates | Dot notation for maps, brackets for lists | Not supported — use full object via `Entity.set()` or `DynamoClient.updateItem` | ElectroDB only |
...
| DynamoDB Streams parsing | `entity.parse()` utility | `Entity.marshalledSchema()`, `Entity.itemSchema()` | Parity |
...
| Optimistic locking | Not built-in | Built-in `Entity.expectedVersion(n)` | **Advantage** |
...
| Error handling | `ElectroError` with codes | 18 tagged errors with `catchTag` discrimination | **Advantage** |
...
5. **Effect integration** — Full Effect ecosystem: Layer-based DI, 18 tagged errors with `catchTag`, Stream-based pagination, resource management, composable pipelines, dual APIs. This is the primary architectural differentiator.

6. **9 derived types** — One entity declaration automatically produces Model, Record, Input, Update, Key, Item, Marshalled types plus ref-aware Input and Update variants. ElectroDB has fewer derived types and no marshalled/item distinction.
...
| Data callback | Pipe composition with Effect operators |
| Nested property updates | Full object via `Entity.set()` or `DynamoClient.updateItem` directly |
