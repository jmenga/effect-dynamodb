...
| Result limiting | `limit: N` (items returned) | `Query.limit(n)` / `.limit(n)` | Parity |
| Page size | `limit` doubles as the per-request size | `Query.pageSize(n)` / `.pageSize(n)` | **Advantage** — result size and round-trip size are separate knobs |
