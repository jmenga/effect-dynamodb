...
| `entity.query.<name>(key).go()` | `bound.collect(EntityDef.query.<name>(key))` | Queries built on entity definition, collected via bound client |
...
| `entity.scan.go()` | `bound.collect(EntityDef.scan())` | Scan returns Query — compose with filter, limit, etc. |
...
**effect-dynamodb:**
```typescript
const { UserEntity: users, TaskEntity: tasks } = yield* DynamoClient.make(MainTable)

// Query by index
const userResults = yield* users.collect(
  UserEntity.query.byEmail({ email: "a@b.com" }),
)

// With sort key condition and filter
const taskResults = yield* tasks.collect(
...
| **Fluent chaining** | `entity.query.byEmail(pk).where(...).go()` | Pipe-based: `query.pipe(Query.where(...))` then `bound.collect(query)` |
