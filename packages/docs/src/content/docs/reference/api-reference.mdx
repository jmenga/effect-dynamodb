...
| `write` | `(...ops) => Effect<void>` | Batch-write any number of items (auto-chunks at 25). Accepts unbound `Entity.put`/`Entity.delete` intermediates AND the bound builders returned by `db.entities.*` |
