...
| `EventStore.bind(stream)` | Resolve `DynamoClient` and `TableConfig` from context and return a `BoundEventStream` whose operations have `R = never`. Use inside `ServiceMap.Service` make effects. |
...
class MatchEventService extends ServiceMap.Service<MatchEventService>()(
