...
Not currently refused, and a known gap: entities with `unique`, `versioned: { retain: true }` or `softDelete` write **one** item through this path, so their sentinel / snapshot / tombstone item is not written. Prefer the entity's own operation for those.
