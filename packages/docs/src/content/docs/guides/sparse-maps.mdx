...
This guide covers the **SparseMap storage primitive** (`storedAs: DynamoModel.SparseMap()`) — flattening a logical `Record<K, V>` into per-entry top-level DynamoDB attributes. It is unrelated to the `'sparse'` value of `indexPolicy` (`indexPolicy: () => ({ attr: 'sparse' })`), which controls whether an *update* drops a *GSI* membership when a composite is missing. The two were spelled the same up to 1.5.0 and that collision was a frequent source of confusion. See [indexPolicy](/effect-dynamodb/guides/index-policy/) if you're looking for GSI membership rules.
</Aside>
