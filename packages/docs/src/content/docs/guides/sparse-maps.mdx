...
<Aside type="caution" title="This 'sparse' is not the indexPolicy 'sparse'">
This guide covers the **SparseMap storage primitive** (`storedAs: DynamoModel.SparseMap()`) — flattening a logical `Record<K, V>` into per-entry top-level DynamoDB attributes. It is unrelated to the `'sparse'` value of `indexPolicy` (`indexPolicy: { pk: 'sparse', sk: 'sparse' }`), which controls whether an *update* drops a *GSI* membership when a half's composites are absent. The two were spelled the same up to 1.5.0 and that collision was a frequent source of confusion. See [indexPolicy](/effect-dynamodb/guides/index-policy/) if you're looking for GSI membership rules.
</Aside>
...
<Aside type="note" title="Migrating from 1.5.0">
1.6.0 renames the opt-in from the magic string `storedAs: "sparse"` to the typed callable `storedAs: DynamoModel.SparseMap()`. The `prefix` option moves from a sibling on `ConfigureAttributes` into the SparseMap config object — pass `DynamoModel.SparseMap({ prefix: "t" })` instead of `{ storedAs: "sparse", prefix: "t" }`. The magic string is no longer recognised; mechanical replacement.
</Aside>
