...
`ConditionalCheckFailed` is on the typed error channel wherever it is reachable: on `create()`, `patch()`, `upsert()` and `deleteIfExists()` (which carry a built-in condition), and on any operation once `.condition(...)` is applied. Either way, `Effect.catchTag` handles it without widening by hand:
