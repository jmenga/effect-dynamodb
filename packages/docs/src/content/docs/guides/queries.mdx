...
It also covers composites whose schema **transforms** between its domain type and what DynamoDB stores. `Schema.BigIntFromString` has a `bigint` domain type and a string wire form, so a `420n` composite is stored as `txn_420`; `.where((t, { eq }) => eq(t.txn, 420n))` encodes the operand the same way and finds it. The same encoding applies to composites supplied to the accessor itself and to `.get()` / `.update()` / `.delete()` keys. A value that cannot be encoded at all — a prefix string against a `bigint` composite, say — is refused with `EDD-9048` rather than silently compared against nothing.
