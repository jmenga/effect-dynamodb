...
| `Collections.make({ name: "ByProject", pk: { composite: ["projectId"] }, ... })` | `db.collections.ByProject(...)` | `{ projectId: string }` |
| `Collections.make({ name: "ByAssignee", pk: { composite: ["assigneeId"] }, ... })` | `db.collections.ByAssignee(...)` | `{ assigneeId: string }` |
