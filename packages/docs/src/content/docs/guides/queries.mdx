...
const projectTasks = yield* tasks.collect(
...
const lo = KeyComposer.composeSortKeyPrefix(
  AppSchema,
  "Task",
  1,
  TaskEntity.indexes.byProject,
  { status: "active", createdAt: startDate },
)
const hi = KeyComposer.composeSortKeyPrefix(
  AppSchema,
  "Task",
  1,
  TaskEntity.indexes.byProject,
  { status: "active", createdAt: endDate },
)
...
const highPriActive = yield* tasks.collect(
  TaskEntity.query.byProject({ projectId: "proj-alpha" }),
  TaskEntity.filter((t, { and, eq }) =>
    and(eq(t.status, "active"), eq(t.priority, "high")),
  ),
)
```
...
const page = yield* TaskEntity.query.byProject({ projectId: "proj-alpha" }).pipe(
  Query.limit(3),
  Query.execute,
)
...
// Cursor-based pagination
const page1 = yield* TaskEntity.query.byProject({ projectId: "proj-alpha" }).pipe(
  Query.limit(3),
  Query.execute,
)

if (page1.cursor) {
  const page2 = yield* TaskEntity.query.byProject({ projectId: "proj-alpha" }).pipe(
    Query.limit(3),
...
// Scan with limit
const firstPage = yield* tasks.collect(
  TaskEntity.scan(),
  Query.limit(3),
)
```
...
const consistent = yield* tasks.collect(
  TaskEntity.scan(),
...
yield* Stream.runForEach(scanStream, (t) =>
  Console.log(`  Scanned: ${t.taskId} — "${t.title}"`),
)
```
...
const consistentTask = yield* TaskEntity.get({ taskId: "t-001" }).pipe(
...
const consistentScan = yield* tasks.collect(
  TaskEntity.scan(),
  Query.consistentRead,
)
```
