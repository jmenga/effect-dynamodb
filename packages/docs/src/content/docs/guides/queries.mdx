...
The operand you pass is a **composite attribute value**, not a raw sort key. The library composes it into the same `$schema#v1#entity#<name>_<value>` shape the write path stores — same serialization, same casing — so `gte(t.createdAt, "2025-02-10")` compares like-for-like against the stored key.
