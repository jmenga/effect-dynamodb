...
```typescript
import { KeyComposer, Query, DynamoClient } from "effect-dynamodb"
...
const activePrefix = KeyComposer.composeSortKeyPrefix(TaskEntity, "byProject", {
  status: "active",
})
yield* tasks.collect(
  TaskEntity.query.byProject({ projectId: "proj-alpha" }),
...
const lo = KeyComposer.composeSortKeyPrefix(TaskEntity, "byProject", {
  status: "active", createdAt: startDate,
})
const hi = KeyComposer.composeSortKeyPrefix(TaskEntity, "byProject", {
...
yield* tasks.collect(
  TaskEntity.query.byProject({ projectId: "proj-alpha" }),
  TaskEntity.filter((t, { gt, and, eq }) =>
    and(gt(t.priority, 3), eq(t.status, "active")),
  ),
)
...
const allTasks = yield* tasks.collect(
  TaskEntity.query.byProject({ projectId: "proj-alpha" }),
)
// Task[] (all matching items across all pages)
```
...
// page.items: Task[] (up to 25 items)
...
yield* stream.pipe(
  ...
  Stream.runForEach((chunk) =>
    Effect.log(`Processing ${Chunk.size(chunk)} tasks`)
  ),
)
```
...
// Stream-based scan
const stream = tasks.paginate(TaskEntity.scan())
...
const task = yield* tasks.get({ taskId: "t-001" }, Entity.consistentRead)

// Consistent read on query
const result = yield* tasks.collect(
  TaskEntity.query.byProject({ projectId: "proj-alpha" }),
  Query.consistentRead,
)

// Consistent read on scan
const all = yield* tasks.collect(
  TaskEntity.scan(),
  Query.consistentRead,
)
```
