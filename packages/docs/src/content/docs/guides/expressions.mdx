...
| `KeyConditionExpression` | Select partition + sort key range | `Entity.query.indexName()`, `Query.where()` |
...
```typescript
// On put
yield* products.put(input, Products.condition((t, { eq }) => eq(t.status, "active")))

// On update (ANDed with optimistic lock)
yield* products.update(
  { id },
  Products.set({ name: "New Name" }),
  Products.expectedVersion(3),
...
yield* products.collect(
  ...
  Products.filter((t, { or, eq }) =>
    or(eq(t.status, "active"), eq(t.status, "pending")),
  ),
)
```
...
```typescript
// SET — assign values
yield* products.update({ id }, Entity.set({ name: "New Name", price: 29.99 }))

// REMOVE — delete attributes
yield* products.update({ id }, Entity.remove(["description", "tempFlag"]))

// ADD — atomic increment (numbers) or union (sets)
yield* products.update({ id }, Entity.add({ viewCount: 1, stock: 50 }))

// Subtract — SET field = field - value
yield* products.update({ id }, Entity.subtract({ stock: 3 }))

// Append — SET field = list_append(field, value)
yield* products.update({ id }, Entity.append({ tags: ["on-sale", "featured"] }))
...
For nested attributes, array elements, and attribute-to-attribute operations, use the path-based combinators with `Entity.pathSet()`, `Entity.pathRemove()`, etc.:

```typescript
// SET nested path
yield* products.update({ id }, Entity.pathSet({
...
yield* products.update({ id }, Entity.pathRemove(["metadata", "temporary"]))

// PREPEND to list
yield* products.update({ id }, Entity.pathPrepend({
  segments: ["tags"],
  value: ["URGENT"],
}))

// if_not_exists — set only if the attribute doesn't exist
yield* products.update({ id }, Entity.pathIfNotExists({
  segments: ["createdBy"],
  value: "system",
}))
```
...
`Query.where()` adds a sort key condition to the `KeyConditionExpression`:

```typescript
import { Query } from "effect-dynamodb"

// Exact match
query.pipe(Query.where({ eq: sortKeyValue }))

// Range comparisons
query.pipe(Query.where({ gt: sortKeyValue }))
query.pipe(Query.where({ gte: sortKeyValue }))
query.pipe(Query.where({ lt: sortKeyValue }))
query.pipe(Query.where({ lte: sortKeyValue }))

// Inclusive range
query.pipe(Query.where({ between: [lo, hi] }))

// Prefix match
query.pipe(Query.where({ beginsWith: prefix }))
```

| Operator | Syntax | DynamoDB |
|---|---|---|
| Equals | `{ eq: value }` | `#sk = :sk` |
| Less than | `{ lt: value }` | `#sk < :sk` |
| Less or equal | `{ lte: value }` | `#sk <= :sk` |
| Greater than | `{ gt: value }` | `#sk > :sk` |
| Greater or equal | `{ gte: value }` | `#sk >= :sk` |
| Between | `{ between: [lo, hi] }` | `#sk BETWEEN :lo AND :hi` |
| Begins with | `{ beginsWith: prefix }` | `begins_with(#sk, :prefix)` |

<Aside type="note">
  Sort key values in effect-dynamodb are **composed strings** that include the schema prefix, entity type, and composite attribute values. You don't construct these manually — use `KeyComposer.composeSortKeyPrefix()` to build the prefix from domain attributes, then pass it to `Query.where({ beginsWith: prefix })`.
</Aside>
