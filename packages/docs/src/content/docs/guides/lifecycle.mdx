...
| `Employees.query.byTenant(...)` | No |
| Collection queries | No |
| `employees.deleted.get(key)` | Yes |
| `Employees.deleted.list(key)` | Yes |
...
GSI queries also return empty — deleted items fall out of indexes:
...
const program = Effect.gen(function* () {
  ...
  // yield* db.Employees.get({ employeeId: "emp-alice" })  // ItemNotFound!
