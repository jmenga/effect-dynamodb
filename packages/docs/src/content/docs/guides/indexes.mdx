...
When you call `DynamoClient.make(MainTable)`, the client auto-discovers that both entities share the `tenantMembers` collection and exposes `db.collections.tenantMembers(...)` as a cross-entity query accessor.
