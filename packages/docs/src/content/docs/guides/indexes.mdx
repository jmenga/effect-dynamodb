...
### GSI Access Patterns via Collections

GSI access patterns are defined using `Collections.make()`. Each collection owns the index definition and member entities specify their SK composites:
...
```typescript
const MyCollection = Collections.make("collectionName", {
