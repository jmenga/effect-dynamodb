...
const { MatchAggregate: matches } = yield* DynamoClient.make(MainTable)

const match = yield* matches.get({ id: "bgt-2025-test-1" })
...
const match = yield* matches.create({
  id: "bgt-2025-test-1",
...
const updated = yield* matches.update(
  { id: "bgt-2025-test-1" },
  (current) => ({
    ...current,
    team1: {
...
// Rename match — cursor.key().replace()
yield* matches.update(
  { id: "match-1" },
  ({ cursor }) => cursor.key("name").replace("New Name"),
)
```
...
yield* matches.update(
  { id: "match-1" },
  ({ state, optic }) => optic.key("name").replace("New Name", state),
)
```
...
yield* matches.delete({ id: "bgt-2025-test-1" })
```
...
const match = yield* matches.get({ id: "test-1" }).pipe(
...
const updatedPlayer = yield* players.update(
  { id: "smith-01" },
...
The [Cricket Match Manager](/effect-dynamodb/tutorials/gamemanager/) tutorial demonstrates all three features in a cricket match domain:
