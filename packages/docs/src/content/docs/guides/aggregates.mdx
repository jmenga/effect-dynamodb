...
A declared `sk.composite` is **authoritative**: it replaces the ref-identifier default rather than extending it, so it decides both uniqueness and the order elements sort in. Include the ref's identifier yourself when you still want it in the key — after hydration it lives under the hydrated object, so name it by dotted path:
