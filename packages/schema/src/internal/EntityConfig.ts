 ...
 *
 * - `string` — override field name only (default ISO-string storage)
 * - `Schema.Top` — override storage only (default field name)
 * - `{ field?, schema? }` — override both field name and storage
 *
 * The `schema` half is a **storage descriptor, not a codec**. System timestamps
 * are generated by the library at write time, never decoded from caller input,
 * so the schema is read only for its `DynamoEncoding` annotation — which wire
 * form to emit. Pass an annotated `DynamoModel` date schema
 * (`DynamoModel.DateString`, `DynamoModel.DateEpochMs`,
 * `DynamoModel.DateEpochSeconds`) or one re-pointed with
 * `.pipe(DynamoModel.storedAs(...))`. A schema without that annotation (e.g.
 * `Schema.Number`, `Schema.DateTimeUtcFromMillis`) is rejected at make() time
 * with `EDD-9044` rather than silently storing an ISO string.
 */
